...
/*   Updated: 2017/05/23 16:55:44 by amoinier         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	t_file			*ft_add_info(t_file *list, struct stat info, char *filename, char *flag)
{
	list->name = filename;
	if (ft_strchr(flag, 'l') || ft_strchr(flag, 't') || ft_strchr(flag, 'r')) {
...
t_file			*get_file_datas(char *flag, char *filename)
{
	...
	if (!(file = ft_create_struct()))
		return (NULL);

	lstat(filename, &info);
	file = ft_add_info(file, info, filename, flag);
	return (file);
}
