...
/*   Updated: 2017/05/23 17:06:03 by amoinier         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void 			list_all_file(char *flag, char *path, char *start)
{
	t_file		*tmp;
	DIR			*id_dir;
...
int				main(int ac, char **av)
{
	...
	nbr_flag = 1;
	flag = get_flag_list(ac, av);

	while (av[nbr_flag] && av[nbr_flag][0] == '-') {
		nbr_flag++;
	}
	...
	while (av[nbr_flag] && av[nbr_flag][0])
	{
		list_all_file(flag, av[nbr_flag], av[nbr_flag]);
		nbr_flag++;
	}
